...
! --------- Simulation Iterations
       ...
       do while ((dt_world.gt.MAX_TEMP_ERROR).and. &
          ...
          !$acc loop gang
          do j=1,local_ny
             !$aeg-omp parallel do simd
             !$acc loop worker
             do i=1,local_nx
                !T_new(i,j)=0.25*(T(i+1,j)+T(i-1,j)+T(i,j+1)+T(i,j-1))
                Tp_new(i,j)=0.25*(Tp(i+1,j)+Tp(i-1,j)+Tp(i,j+1)+Tp(i,j-1))
             end do
             !$aeg-omp end parallel do simd
             !$acc end loop
          end do 
          !$aeg-omp end teams distribute
          !$aeg-omp end target
          !$acc end loop
          !$acc end parallel
          ...
          !$acc loop gang reduction(max:dt)
          do j=1,local_ny
             !$aeg-omp parallel do simd reduction(max:dt)
             !$acc loop worker reduction(max:dt)
             do i=1,local_nx
