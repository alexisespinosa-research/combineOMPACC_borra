...
!$aeg-omp           target enter data map(to:T) map(alloc:T_new)
!$omp           target enter data map(to:T(:GRIDX+2,:GRIDY+2)) map(alloc:T_new(:GRIDX+2,:GRIDY+2))
#            else
!$aeg-acc           enter data copyin(T) create(T_new)
!$acc           enter data copyin(T(:GRIDX+2,:GRIDY+2)) create(T_new(:GRIDX+2,:GRIDY+2))
#            endif
#         else
#            if defined(_JUSTOMP_) || defined(_PRELOADOMP_)
...
#            ifndef _JUSTOMP_ 
!$aeg-acc           parallel loop copy(T) copyout(T_new) collapse(2)
!$acc           parallel loop copy(T(:GRIDX+2,:GRIDY+2)) copyout(T_new(:GRIDX+2,:GRIDY+2)) collapse(2)
#            else 
!$aeg-omp           target teams map(tofrom:T) map(from:T_new)
...
#            ifndef _JUSTACC_ 
!$aeg-omp           target teams map(tofrom:T) map(to:T_new) map(tofrom:dt) reduction(max:dt)
...
#            else 
!$aeg-acc           parallel loop copy(T) copyin(T_new) reduction(max:dt) collapse(2)
...
!$aeg-acc           exit data copyout(T)
!$acc           exit data copyout(T(:GRIDX+2,:GRIDY+2))
#            else
!$aeg-omp           target exit data map(from:T)
!$omp           target exit data map(from:T(:GRIDX+2,:GRIDY+2))
#            endif
#         else
#            if defined(_JUSTACC_) || defined(_PRELOADACC_)
