       ...
       !$aeg-omp target enter data map(alloc:T_new)
       !$aeg-omp target data map(to:T)
       ...
       do while ((dt.gt.MAX_TEMP_ERROR).and. &
          ...
          !reset dt
          dt=0.0 
          !$omp target map(tofrom:T) map(tofrom:dt)
          !$aeg-omp target map(tofrom:dt)
          ...
          if (mod(iteration,50).eq.0) then
             ...
             print "(a,E15.8)",'T(0.9,0.9)=',T(int(GRIDX*0.9),int(GRIDY*0.9))
          end if  
